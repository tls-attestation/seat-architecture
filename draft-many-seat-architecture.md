...
: An asymmetric key pair whose private key is generated
  ...
  (e.g., a TEE). If a deployment's policy requires Compound
  Authentication (see {{security-considerations}}), where the
  transport authentication key itself natively proves Target
  Environment residency, a `tbK` MUST be used to authenticate
  the Attester’s transport endpoint (for example, signing the
  TLS CertificateVerify message), and Evidence produced by the
  Attesting Environment MUST include a binding to the `tbK`.
...
: An asymmetric Key Encapsulation Mechanism (KEM) key pair generated
  ...
  As a per-session liveness anchor, the `keK` is ephemeral and closely tied
  ...
  Regularly rotating the `keK` in this role strictly bounds the
  vulnerability window of an exfiltrated key to the current validity
  period.

> The Verifier includes the public `keK` within the Attestation Result,
...
> Alternatively, the `hbK` MAY be provisioned directly into the
> Target Environment as the transport authentication key itself.
> This pattern can be applied for deployments such as hardened
> external Key/Secret Management Services (KSMs) or certificate
> managers. Using an injected hbK shifts the security anchor from a
> hardware-backed guarantee to the integrity of the local
> provisioning pipeline and does not carry the exclusivity or
> non-exportability guarantees of a natively generated `tbK`.

> To cryptographically anchor the session to the Target Environment,
> a deployment MUST employ at least one natively generated key
> (either a `tbK` or a `keK`).
...
A Background-Check exchange MAY also be used to provision a `keK`
for subsequent use under the Passport model, by minting a cached
Attestation Result that binds a Trust-On-First-Use `keK` for later
...
Where the `keK` instead serves as a per-machine continuity anchor pinned
...
successive `keK` MUST be attested by the outgoing `keK` at the point of
rotation, so that the Relying Party can verify an unbroken chain of
succession back to the originally pinned `keK` without requiring a fresh
...
Where the transport authentication key is a non-confidential `hbK`
provisioned directly into the Target Environment rather than a
natively generated `tbK`, binding the Attestation Credential to that
...
transport authentication key. If the policy requires Compound
Authentication, the Attestation Credential MUST be bound to a
Target/TEE-Bound Key (`tbK`) generated and maintained exclusively
within the Target Environment. Conversely, if the policy accepts
independent verification of endpoint identity and platform state,
a Hardware-Bound Key (`hbK`) injected into the Target Environment MAY
be used for transport authentication. However, because the injected
`hbK` lacks native non-exportability guarantees, binding the
Attestation Credential to that key's public component alone does
not establish that the key is exclusive to the attested environment.
In this case, an additional anchor, such as a KEM Encapsulation
Key (`keK`) bound to the session per {{interactive-passport}}, is
REQUIRED to prevent substitution of the binder by an untrusted
host holding the same injected key material.
