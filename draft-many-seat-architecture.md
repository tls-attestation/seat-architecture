...
> Note that while Evidence is exclusively generated by the
> Attester, Attestation Results are generated by the Verifier.
> Depending on the conveyance model in use, an Attestation Credential
> may be conveyed to the Relying Party directly by the Verifier
> (Background-Check Model) or presented by the Attester after
> obtaining it from a Verifier (Passport Model).
